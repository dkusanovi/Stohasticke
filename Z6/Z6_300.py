...
def Psi(r):  
    return (27 - 18*r + 2*r**2) * np.exp(-r/3.0)  # 300, valna funkcija bez ovisosti o z
...
# return z*np.exp(-r/2) # 210

# ik = 0                                                # indeks koraka, trenutni korak (index of step)
# ib = 1                                                # indeks bloka, trenutni blok (index of block)
x = np.random.uniform(-15.0, 15.0, (3, Nw))             # polozaji [x, y, z] setaca
# xp = np.zeros(4)                         # koordinate probnog polozaja setaca (trial position)
dX = np.array([5.0, 5.0, 5.0])             # maksimalna duljina koraka (maximum change of coordinate)
...
ff = open("r_300.dat", "w")                   # prosjecni radijus <r>
frNw = open("rNw_300.dat", "w")               # polozaj svakog setaca
